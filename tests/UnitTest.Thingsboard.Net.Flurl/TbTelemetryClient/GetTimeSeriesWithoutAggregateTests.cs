...
public class GetTimeSeriesWithoutAggregateTests
{
    ...
    public async Task TestGetTimeSeries()
    {
        ...
        var field2 = "humidity";
        await client.SaveEntityTelemetryAsync(TbEntityType.DEVICE, TbTestData.TestDeviceId, new Dictionary<string, object> {{field1, 10}, {field2, 20}});

        // act
        var entities = await client.GetTimeSeriesAsync(TbEntityType.DEVICE, TbTestData.TestDeviceId, new[] {field1, field2}, DateTime.Today.AddDays(-7), DateTime.Now);
    ...
    public async Task TestGetTimeSeriesThatDoesNotExists()
    {
        ...
        // act
        var entities = await client.GetTimeSeriesAsync(TbEntityType.DEVICE, TbTestData.TestDeviceId, new[] {"test_not_exists"}, DateTime.Today.AddDays(-7), DateTime.Now);
